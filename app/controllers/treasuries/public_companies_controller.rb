    ...
    public_company = %Q(
      <li class="flex">
        <div class="flex items-center">
          ...
          <a href="/treasuries/countries/#{@public_company.name}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700" aria-current="page">#{@public_company.name} (#{@public_company.long_ticker})</a>
        </div>
      </li>
    ).html_safe
